...
class PFile
{
	 ...
	 */
	static function download($url, $ext = NULL, $filename = NULL, $directory = NULL, $chmod = 0644, $referer = NULL) {
		if (!$referer) {
			$referer = $url;
		}
		$ch = curl_init($url);
		...
		if (!$ext) $ext = self::mime2ext($chinfo['content_type']);
		if (!$ext) $ext = self::path2ext($chinfo['url']);
		$ext = strtolower($ext);
		if ($ext) $fext = '.'.$ext;
		...
		$filename = realpath($directory).'/'.$filename.$fext;
		if (file_put_contents($filename, $ret)) {
